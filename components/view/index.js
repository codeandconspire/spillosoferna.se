  ...
  return function (state, emit) {
    return state.prismic.getSingle('website', function (err, doc) {
      ...
      function closeGoals (event) {
        emit('pushState', state.beforeGoalsPage)
        window.requestAnimationFrame(() => window.scrollTo(0, state.beforeGoalsScroll))
      ...
      return html`
        <body class="View ${props.floating ? 'View--floating' : ''}">
          ${!props.floating ? state.cache(Header, 'header').render(state.href) : html`
            <a class="View-close" onclick=${closeGoals} href="/start">
              Stäng
